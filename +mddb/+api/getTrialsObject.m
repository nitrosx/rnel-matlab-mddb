%
%
function [trials,extras] = getTrialsObject(query)
    % function [trials,extras] = mddb.api.getTrialsObject(query)
    %
    % retrieve full trials info list according to query values
    %
    % inputs
    ...
    % calls getTrials
    [trials,tExtras] = mddb.api.getTrials(query);
