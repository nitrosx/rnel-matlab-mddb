...
function result = scan_yaml(r)
    % function result = mddb.lib.yaml.scan_yaml(r)
    %
    % input
    % - (java object) r
    %
    % output
    % - (matlab object) result
...
function result = scan_list(r)
    ...
    while it.hasNext()
        i = it.next();
        result{ii} = mddb.lib.yaml.scan_yaml(i);
        ii = ii + 1;
    end;
end
...
function result = scan_map(r)
    ...
    while it.hasNext()
        ...
        ich = char(i);
        if iskw_import(ich)
            result.(ich) = mddb.lib.yaml.perform_import(r.get(java.lang.String(ich)));
        else
            result.(genvarname(ich)) = mddb.lib.yaml.scan_yaml(r.get(java.lang.String(ich)));
        end;
    end;
